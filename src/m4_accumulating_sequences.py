...
def make_simple_string(m, n):
    ...
    simple_string = ''
    for k in range(n - m + 1):
        simple_string = simple_string + str(m + k) + '-'
    print(simple_string)
...
def run_test_rectangles_from_circles():
    ...
    rectangles = rectangles_from_circles(circles, window)
...
def rectangles_from_circles(circles, window):
    """
    ...
    for k in range(len(circles)):
        c1 = rg.Point(circles[k].center.x - circles[k].radius,
                      circles[k].center.y - circles[k].radius)
        c2 = rg.Point(circles[k].center.x + circles[k].radius,
                      circles[k].center.y + circles[k].radius)
    ...
    for k in range(len(circles)):
        rectangle_list = rectangle_list + [circles[k]]
    window.render(0)
